...
public class ClientModel {
	//2. getInstance method에서 호출된 instance가 ClientModel의 instance임을 정의
	private static ClientModel instance = new ClientModel();
	
	//1. private 생성자 만들고 instance 부를 수 있는 getInstance method 만들기
	private ClientModel() {}
	...
	public ClientDTO insert(ClientDTO v) {
	...
	public ClientDTO search(String name) {
		ClientDTO v = null;
		for(int i=0; i<clientDB.length; i++) {
			v = clientDB[i];
	...
	public ClientDTO update(ClientDTO clientInfo) {
		for(int i=0; i<clientDB.length; i++) {
